-- ============================================================
-- Oublex — Admin close-game support
--
-- Run in Supabase → SQL Editor → New Query, ONCE the oublex_games
...
--      columns on oublex_games
--   2. oublex_admin_close_game(uuid, text) RPC — soft-closes a
--      game with no winner attribution; reason is REQUIRED
--   3. oublex_admin_list_open_games() RPC — lists open + active
--      games for the admin panel UI
--   4. oublex_admin_list_closed_games() RPC — lists recently
--      closed games with the closing admin's name + reason
--
-- If your game's table is named differently (e.g. oublex_matches),
-- find/replace `oublex_games` below before running.
-- ============================================================
...
create or replace function public.oublex_admin_close_game(
  p_game_id uuid,
  p_reason  text
)
...
grant execute on function public.oublex_admin_close_game(uuid, text) to authenticated;

-- ── 3. oublex_admin_list_open_games ─────────────────────────
-- Returns waiting/active games for the admin Close Games panel.
-- Joins through your players table to surface usernames; rename
-- oublex_players + the join column if your schema differs.
create or replace function public.oublex_admin_list_open_games()
returns table (
  id           uuid,
  status       text,
  ...
  left join public.oublex_players gp on gp.game_id = g.id
...
grant execute on function public.oublex_admin_list_open_games() to authenticated;

-- ── 4. oublex_admin_list_closed_games ──────────────────────
-- Returns recently closed games with closing admin's username +
-- reason. Used by the admin panel's "Recently Closed" history view.
create or replace function public.oublex_admin_list_closed_games(p_limit int default 50)
returns table (
  id              uuid,
...
grant execute on function public.oublex_admin_list_closed_games(int) to authenticated;
