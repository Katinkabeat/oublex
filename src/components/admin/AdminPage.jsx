...
// in `permissions`.
//
// Wires to two RPCs created by supabase/migrations/oublex_admin_close_game.sql:
//   - oublex_admin_list_open_games() — list waiting/active games
//   - oublex_admin_close_game(uuid)  — soft-close a game (no winner)
//
...
export default function AdminPage({ session, profile, isAdmin }) {
  ...
  const loadGames = useCallback(async () => {
    ...
    if (error) {
      console.error('oublex_admin_list_open_games failed:', error)
  ...
  async function closeGame(gameId) {
    setClosingId(gameId)
    try {
      const { error } = await supabase.rpc('oublex_admin_close_game', { p_game_id: gameId })
