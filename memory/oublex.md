...
Dino reported the fail screen had no leaderboard link "like the win one does";
in fact **neither** did. Root cause: **c240** ("End-of-daily → leaderboard
shortcut, all games") marked Oublex "NO WORK NEEDED — already ships the exact
pattern," but the buttons it found were on SoloGamePage's "You've delved today"
**re-entry panel**, not on the run's end screen. Different surface, so the sweep
closed a game it had never actually covered. Yahdle/Rungles/Snibble all put the
row on the end screen itself and never gate it by outcome or save state.
Commit `b9ba163`.
...
## 2026-07-09 — Fix silent game-over write failure → replay-loop (Snuggie report)
**Symptom:** Snuggie killed the boss but her score never hit the leaderboard and she could "play again" (2–3× in a day). Root cause: `handleGameOver` fired the record RPC **fire-and-forget** (only `console.error` on failure). When the write failed — almost always a **stale/expired access token after a backgrounded mobile tab** (supabase-js's refresh timer gets throttled) → PostgREST 401 — the score was silently dropped AND, since the snapshot is only deleted on RPC success, her pre-kill run sat in `oublex_daily_runs` forever, so every reopen **resumed her at the boss** = the replay loop. Only became visible once the resume feature shipped (07-02); before that a failed write just re-rolled a fresh dungeon (invisible). Diagnosed from prod DB: confirmed her boss-kill left no result row + a frozen boss-2HP snapshot; RPC itself is healthy (fresh authed user records fine, HTTP 204).
- **Fix (`SoloGamePage.jsx` + `OublexGame.jsx`):** `recordResult()` now `await`s the RPC in a 3-try loop; on failure it calls `supabase.auth.refreshSession()` (renews the stale token) + backs off, then retries. A `saveState` (idle|saving|error|saved) is threaded to the end screen: shows "Saving…", then "logged" only when truly saved, or "Couldn't save your run — Retry" (button) on hard failure. Snapshot intentionally stays until a write succeeds, so a run is never lost. Commit 7a3a093.
- **Verified locally (service_role session-injection on standalone dev, boss-2HP snapshot seed):** happy path → kill → result row written (147, cleric) + snapshot deleted + "logged" shown. Self-heal chain proven at the API layer: refresh_token grant mints a new access token that records via the RPC (204) — i.e. attempt-1-401 → refresh → attempt-2-204. All test data cleaned up after.
- **This closes the "Rae to confirm a full run once" flag** from the 07-02 note — a full authed run now clicks through and records.
- **Snuggie unblock:** no fabricated score — cleared her stuck 07-09 snapshot so she redoes today once on the fixed build (Rae's call).
...
**Oublex SW:** bumped `oublex-v1 → v2`. Push fn: `supabase/functions/oublex-push-notification/index.ts`. Deployed + smoke-tested 200.
